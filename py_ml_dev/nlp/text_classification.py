...
def evaluate_model(model, X, y, label: str = "") -> float:
...
def main(name="simple_text_classification") -> None:
    ...
    if os.path.exists(path_X_train_cleaned):
        ...
        print(
            f"Cleaning training data took {(datetime.now() - start_time).seconds} seconds."
        )
    ...
    print(
        f"Reduced training data to {n_components} dimensions using PCA. "
    ...
    X_test_counts = vectorizer.transform(
        [" ".join(tokens) for tokens in X_test_cleaned]
    )
    ...
    with mlflow.start_run(run_name=name + "-log-reg", log_system_metrics=True):
        ...
        print(
            "Training Logistic Regression with CV took",
            (datetime.now() - start_time).seconds,
            "seconds",
        )
        evaluate_model(
            clf_logistic, X_train_tfidf, y_train, "LogisticRegressionCV (train)"
        )
        evaluate_model(
            clf_logistic, X_test_tfidf, y_test, "LogisticRegressionCV (test)"
        )
    ...
    with mlflow.start_run(run_name=name + "-xgboost", log_system_metrics=True):
        ...
        print(
            "Training XGBoost with RandomizedSearchCV took",
            (datetime.now() - start_time).seconds,
            "seconds",
        )
        evaluate_model(
            xgb_search.best_estimator_, X_train_tfidf, y_train, "XGBoostSearch (train)"
        )
        evaluate_model(
            xgb_search.best_estimator_, X_test_tfidf, y_test, "XGBClassifier (test)"
        )
